...
userController.createUser = (req, res, next) => {
  ...
  User.create({ username: username, password: password }) //INSTEAD of doing newUser = new User ({obj}) then .save that--> User.create makes AND saves a new instance of User schema
    ...
    .catch((err) => {
      ...
      const error = {
        log: 'There was an error creating new user in database.',
      };
      next(error);
    });
};
...
userController.shareMap = (req, res, next) => {
  ...
  if (!username || !mapName || !mapData) {
    return res
      .status(400)
      .json({ message: 'Error: Incomplete data provided.' });
  }
  ...
  User.findOne({ username })
    ...
    .then((user) => {
      if (!user.sharedMaps[mapName]) {
        return res
          .status(404)
          .json({ message: 'Error: Could not find entry in database.' });
      }
...
userController.addFriend = (req, res, next) => {
  ...
  User.findOne({ username: friend })
    ...
    .catch((err) => {
      const error = {
        log: 'Failed during userController.addFriend.',
        message: err.message,
      };
      return next(error);
    });
