...
const DB_URI =
  'mongodb+srv://staskusscott:iTCnI0Xt8H94Skjz@cluster0.rxxfq3r.mongodb.net/?retryWrites=true&w=majority';
mongoose
...
app.post('/signup', userController.createUser, (req, res) => {
  ...
  res.status(200).json({
    success: true,
    message: `Sign up successful! Welcome, ${res.locals.user.username}, to the wild world of caves`,
  });
});
...
app.get('/friendmaps/:username', userController.getFriendMaps, (req, res) => {
  res.status(200).json(res.locals.friendListMaps);
});

app.get(
  '/mapcollection/:username',
  userController.getPrivateMaps,
  (req, res) => {
    res.status(200).json({
      message: 'Maps gathered from private collection.',
      success: true,
      maps: res.locals.mapCollection,
    });
  }
);
...
app.use((req, res) =>
  res.status(404).send(`Couldn't find the page you're looking for. . .`)
);
